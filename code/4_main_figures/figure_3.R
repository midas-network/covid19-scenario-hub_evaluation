...
l = ggplot(data= data.frame(x = 1:3, y = 1:3)) + 
  geom_line(aes(x=x, y = y, color = "4-week forecast"), size = 1.2) + 
...
pC <- skill_byround[setDT(model_name_key_nat), on = "model_name" ] %>%
  ...
  geom_text(aes(label = round(skill, 1)), size = 2.5) + 
...
ggsave("figures/main_figures/figure3.pdf", width = 10, height = 8)
